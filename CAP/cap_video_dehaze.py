...
def dehaze_video(input_path, output_path, width=640, height=360):
    ...
    print(f"🎥 Input: {input_path}, Total frames: {total_frames}, Target size: {width}x{height}")
    ...
    while True:
        ...
        cv2.putText(
            dehazed,
            f"FPS: {avg_fps:.2f} | ETA: {int(eta)}s",
            (10, 30),
        ...
        cv2.imshow("CAP Dehazed", dehazed)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Dehaze video using Color Attenuation Prior (CAP)")
